...
//          if ( !(a[j/32]&BITMASK_1[j%32]) ){
    ...
    if ( m <= th ){
      ...
      if ( m == 0 ){
        ARY_INS (PP->II.itemset, *x);
        ITEMSET_output_itemset (&PP->II, NULL, 0);
        PP->II.itemset.t--;
        PP->itemchr[*x] = 4;
        ARY_INS (PP->itemjump, *x);
      }
    } else if ( m >= PP->FF.t ){ PP->itemchr[*x] = 4; ARY_INS (PP->itemjump, *x); }
    else PP->itemchr[*x] = 1;
  }
}
...
int SHD_update (PROBLEM *PP, QUEUE_INT e){
  ...
  if ( PP->BA.v && PP->num < PP->OQ[e].t ){  // bitmap version, when bitmap matrix exists, and PP->OQ[e] is larger than crit
    ...
    MALIST_DO_FORWARD (*A, PP->FF.clms, j){
     if ( a[j/32]&(1<<(j&31)) ){
        A->list[j] = e;
        ARY_INS (PP->vecjump, A->prv[j]);  // undo list for uncov
    ...
    MQUE_FLOOP (PP->II.itemset, t){
      MALIST_DO_FORWARD (*A, *t, j){
       if ( a[j/32]&(1<<(j&31)) ){
          ...
          A->prv[A->nxt[j]] = A->prv[j];
          PP->vecchr[j] = 1;
          ARY_INS (PP->veccand, j);  // undo list for crit
        }
      }
    }
    ...
    MQUE_FLOOP (PP->OQ[e], t){
      if ( A->list[*t] == PP->FF.clms ){
        A->list[*t] = e;
        ARY_INS (PP->vecjump, A->prv[*t]);  // undo list for uncov
        ...
        A->prv[A->nxt[*t]] = A->prv[*t];
        PP->vecchr[*t] = 1;
        ARY_INS (PP->veccand, *t);  // undo list for crit
      }
    }
  }
...
void SHDC_crit_check (PROBLEM *PP, QUEUE_INT tt, QUEUE_INT prv){
  ...
  FLOOP (item, 0, PP->FF.clms){
    ...
    if ( m <= tt ){
      if ( u == PP->FF.t ){
        ARY_INS (PP->II.itemset, item);
        ITEMSET_output_itemset (&PP->II, NULL, 0);
        PP->II.itemset.t--;
        PP->itemchr[item] = 4;
        ARY_INS (PP->itemjump, item);
      }
    } else if ( m >= PP->FF.t ){ PP->itemchr[item] = 4; ARY_INS (PP->itemjump, item); }
...
//      FLOOP (i, 0, PP.FF.clms) ARY_INS (PP.veccand, i);
    } else {
